...
If you are tracking Emacs directly from its =master= branch, note that you
do not need to install any package as the themes are already built in
(starting with commit =c1b0515=).  Otherwise you are running a stable
version of Emacs, in which case you should continue reading this
section.
